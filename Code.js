 ...
 */
function saveGroup(groupName, eleveIds) {
  try {
    if (!groupName || !Array.isArray(eleveIds)) {
    ...
    if (!header) {
      return { success: false, error: 'Impossible de construire l\'index des élèves' };
    }
    ...
    const rowData = eleveIds.map(id => rows[id] || [id]);
    if (rowData.length > 0) {
      const maxCols = Math.max(header.length, ...rowData.map(r => r.length));
    ...
    console.log(`✅ Groupe ${groupName} sauvegardé avec ${eleveIds.length} élèves`);
    return { success: true, message: `Groupe ${groupName} sauvegardé`, count: eleveIds.length };
  } catch (e) {
